...
#' adj[1, 2] <- adj[2, 1] <- 0
#' diag(adj) <- 1
#' result <- compute_maximal_partial_clique(adj, 0.9)
#' print(result)
compute_maximal_partial_clique <- function(adj_mat, alpha) {
  ...
  edge_density <- function(subgraph) {
    m <- length(subgraph)
    if (m <= 1) return(1)
    total_edges <- (sum(adj_mat[subgraph, subgraph]) - m) / 2
    max_edges <- m * (m - 1) / 2
    total_edges / max_edges
  }

  # Greedy Heuristic Search Helper function (GPT Assisted)
  find_partial_clique_from_node <- function(start_node) {
  ...
  best_clique <- c(1)
  for (start in 1:n) {
    clique <- find_partial_clique_from_node(start)
  ...
  # Final density
  final_density <- edge_density(best_clique)
