...
- I have read and understood the entire syllabus, no questions.
...
- STAT 34x (R), STAT 425 (R), CSE 447 (Python).
...
- I hope to learn more about effectively use HPC and sending cluster jobs, how to properly monitor efficiency in coding.
...
- The function takes an input of n which is an integer. Then, for each value from 0 till n, it will randomly select a random distribution from N(10,1), Gamma(2,2), and Chisq(3) to pull a random value from. Finally, the output of these distributions are returned.
...
```{r}
# Continue to set seed as 1
set.seed(1)

# Generate the data "trials" amount of times and return the emperical mean.
generation_helper <- function(n, trials) {
  means <- numeric(trials)
  for (i in 1:trials) {
    means[i] <- mean(generate_data(n))
  }
  return(means)
}

# Repeated generate data trials.
mean1 <- generation_helper(1, 10000)
mean2 <- generation_helper(2, 10000)
mean3 <- generation_helper(3, 10000)
mean5 <- generation_helper(5, 10000)
mean10 <- generation_helper(10, 10000)
mean100 <- generation_helper(100, 10000)

# Make Graphs.
par(mfrow = c(3, 2))
hist(mean1, breaks = 100, main = "n = 1", xlab = "Empirical Mean")
hist(mean2, breaks = 100, main = "n = 2", xlab = "Empirical Mean")
hist(mean3, breaks = 100, main = "n = 3", xlab = "Empirical Mean")
hist(mean5, breaks = 100, main = "n = 5", xlab = "Empirical Mean")
hist(mean10, breaks = 100, main = "n = 10", xlab = "Empirical Mean")
hist(mean100, breaks = 100, main = "n = 100", xlab = "Empirical Mean")
```
- All done! See above for my code.

<u>**Question 2C**</u>: 
In one to three sentences, write how the plot you reproduced helps to verify 
the Central Limit Theorem.

- We see that despite the data generated randomly comes from three separate distribution, we start to see a more normal distribution as we increase n, which confirms the central limit theorem.
...
- The head(df) displays the first few rows (6 in this case) of the data set, this gives us a peak at what kind of data we are dealing with. The summary(df) displays the summary statistics for each of the columns, allowing us to get a better understanding on what the distribution of each feature looks like.
...
- The class of `df` is a "data.frame".
...
- All done! The summary here in Question3E is more informative than the summary show at the start of Question 3 as we have the count of each factor instead of just how many total character fields there are for a `character` field.
...
CASI.quantile.groups <- cut(df$Last.CASI.Score, breaks = quantiles)
table(CASI.quantile.groups, df$Cognitive.Status)
```

- Group with higher CASI tend to have less dementia compared to the groups with lower CASI.
...
- No Feedback! Thank you for the detailed tutorial.
