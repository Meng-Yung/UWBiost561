...
- All done! Used GPT to help develop this function. 
...
- All done! Checked if working (from tutorial), and if works with different extreme sizes. 
...
- All done! I created one test per "List of types of unit-tests" from the lecture
...
- All done! devtools::test() results in all tests passed.
...
- I tried to debug this many times and have failed to do so on my laptop, I believe it might have something to do with Quarto not having an implementation for Windows ARM. When I first ran "devtools::check()", it fails when "creating vignettes" giving me an error of "in `(function (command = NULL, args = character(), error_on_status = TRUE, …`:! System command 'Rcmd.exe' failed". Digging deeper by running "Rcmd.exe" in command prompt, it resulted in telling me that Quarto can not be found. I then ran "devtools::check(vignettes = FALSE)" to skip that step, and the final error further confirmed my suspicion with : "   ERROR: Unknown command "TMPDIR=C:/Users/myung/AppData/Local/Temp/Rtmp6r1g7W/file892c5d70804". Did you mean command "uninstall"?
   Warning in system2("quarto", "-V", stdout = TRUE, env = paste0("TMPDIR=",  :
     running command '"quarto" TMPDIR=C:/Users/myung/AppData/Local/Temp/Rtmp6r1g7W/file892c5d70804 -V' had status 1". After a few hours of trying to fix this, I think I can get this working in another Operating environment, but I do not have one set up at the moment. I will include screen shots in the following question. 
...
- Everything passes except for quarto as mentioned before:

```{r}

knitr::include_graphics("../images/check1.png")
knitr::include_graphics("../images/check2.png")
knitr::include_graphics("../images/check3.png")
```
...
- Makes sense! Thank you for the preview! I don't have any previous R projects due to limited coding courses in R, so I most likely will use the default packaging... 
...
- Due to how "new" ARM windows is, I failed to find a solution for quarto after hours of investigating...  I will switch to a different developing environment (non-ARM Windows) for the Homework 4 and the Final project. Thank you!
