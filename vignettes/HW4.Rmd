...
test_that("Output format is correct", {
  ...
  res <- compute_maximal_partial_clique3(adj, 0.6)
...
test_that("Edge density is in the correct range", {
  ...
  res <- compute_maximal_partial_clique3(adj, 0.7)
...
test_that("Function works on various valid inputs", {
  ...
  for (size in sizes) {
    for (alpha in alphas) {
      ...
      res <- compute_maximal_partial_clique3(adj, alpha)
      expect_true(is.numeric(res$clique_idx))
...
test_that("Function gives correct result on a crafted input", {
  ...
  res <- compute_maximal_partial_clique3(adj, 1.0)
...
test_that("Function identifies clique from generated partial graph", {
  ...
  result <- compute_maximal_partial_clique3(adj, alpha = clique_density)
...
test_that("Output format is correct", {
  ...
  res <- compute_maximal_partial_clique8(adj, 0.6)
...
test_that("Edge density is in the correct range", {
  ...
  res <- compute_maximal_partial_clique8(adj, 0.7)
...
test_that("Function works on various valid inputs", {
  ...
  for (size in sizes) {
    for (alpha in alphas) {
      ...
      res <- compute_maximal_partial_clique8(adj, alpha)
      expect_true(is.numeric(res$clique_idx))
      expect_gte(res$edge_density, alpha)
    }
  }
...
test_that("Function gives correct result on a crafted input", {
  ...
  res <- compute_maximal_partial_clique8(adj, 1.0)
...
test_that("Function identifies clique from generated partial graph", {
  ...
  result <- compute_maximal_partial_clique8(adj, alpha = clique_density)
...
- I will be using the `generate_partial_clique()` function, where it will generate random graphs with   clique_fraction = 0.5 and clique_edge_density = 0.95, with varying n.
...
- I am choosing to vary the random graphs across n=5 to n=10, and doing 2 trials for each n.
...
- I am hoping to be able to learn more holistically how each implementation performs (returns correct maximal partial clique) as n increases from 5 to 10.

- 5) How many trials do you intend to run per "level"?

- I intend to do 10 trials per "level" (n).

- 6) How much time do you estimate your simulation study will need at maximum? See Note 2 below.

- I estimate that my simulation study will need at most 30 seconds * 15 implementations  = 7 1/2 minutes per trial, where 10 trials will take about at most an hour ish, as I am going through levels of n that is relatively small.
