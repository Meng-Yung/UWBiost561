...
- I was assigned implementations \# 3 and \# 8. Here are my quick summary of what I think each implementation does.

- For implementation \# 3:  This implementation searches for the largest subset of nodes (up to max size 10) in an undirected graph (represented by an adjacency matrix) that forms a partial clique—a subgraph with edge density greater than or equal to a given threshold alpha (between 0.5 and 1). It checks all combinations of nodes from largest (up to 10) down to size 2 and returns the first one that meets the density criterion. The function ensures that the input matrix is symmetric, binary, includes self-loops (diagonal ones), and is of size between 5 and 50. It returns the indices of the selected nodes and the corresponding edge density. 

- For implementation \# 8: This implementation attempts to find a subset of nodes in a graph (given by an adjacency matrix) that forms a partial clique with edge density at least alpha, using a node-peeling approach. It iteratively removes the node with the fewest connections until the remaining subgraph meets the edge density threshold or only one node remains. The helper functions .peel_node_8 and .obtain_density_8 handle node removal and edge density calculation.
...
- For implementation \# 3: This code was relatively easy to understand to me as my implementation was very similar. I would recommend for the for loops and if statements to comment explicitly what the structure would look like (i.e. what does m stand for?) for those who are not familiar with the problem.

- For implementation \# 8: This code was a bit more messy in terms of trying to understand. Despite it looking relatively elegant with helper functions, it is not clear exactly what is happening at first glance, and there is no comments to see why helper functions were needed.
...
test_that("Output format is correct", {
  ...
  res <-  UWBiost561::compute_maximal_partial_clique3(adj, 0.6)
...
test_that("Edge density is in the correct range", {
  ...
  res <-  UWBiost561::compute_maximal_partial_clique3(adj, 0.7)
...
test_that("Function works on various valid inputs", {
  ...
  for (size in sizes) {
    for (alpha in alphas) {
      ...
      res <-  UWBiost561::compute_maximal_partial_clique3(adj, alpha)
      expect_true(is.numeric(res$clique_idx))
...
test_that("Function gives correct result on a crafted input", {
  ...
  res <-  UWBiost561::compute_maximal_partial_clique3(adj, 1.0)
...
test_that("Function identifies clique from generated partial graph", {
  ...
  gen <-  UWBiost561::generate_partial_clique(n = n,
  ...
  result <-  UWBiost561::compute_maximal_partial_clique3(adj, alpha = clique_density)
...
- Implementation \# 3 passed all my tests!
...
test_that("Output format is correct", {
  ...
  res <-  UWBiost561::compute_maximal_partial_clique8(adj, 0.6)
...
test_that("Edge density is in the correct range", {
  ...
  res <-  UWBiost561::compute_maximal_partial_clique8(adj, 0.7)
...
test_that("Function works on various valid inputs", {
  ...
  for (size in sizes) {
    for (alpha in alphas) {
      ...
      res <-  UWBiost561::compute_maximal_partial_clique8(adj, alpha)
      expect_true(is.numeric(res$clique_idx))
      expect_gte(res$edge_density, alpha)
    }
  }
...
test_that("Function gives correct result on a crafted input", {
  ...
  res <-  UWBiost561::compute_maximal_partial_clique8(adj, 1.0)
...
test_that("Function identifies clique from generated partial graph", {
  ...
  result <-  UWBiost561::compute_maximal_partial_clique8(adj, alpha = clique_density)
...
- Implementation \# 8 passed all my tests!
...
```{r}
# Generate a complex partial clique graph with 50 nodes
set.seed(123)
data_large <- UWBiost561::generate_partial_clique(
  n = 20,
  clique_fraction = 0.5,
  clique_edge_density = 0.95
)

# Run the maximal partial clique computation with a very short time limit
# Checked that it works at time_limit = 30
set.seed(123)
result_timeout <- UWBiost561::compute_maximal_partial_clique_master(
  adj_mat = data_large$adj_mat,
  alpha = 0.95,
  number = 15,        
  time_limit = 30
)
...
- Alright, I deliberately caused a timed out status using implementation 15.
...
- I will be using the `generate_partial_clique()` function, where it will generate random graphs with   clique_fraction = 0.5 and clique_edge_density = 0.5, with varying n.
...
- I am choosing to vary the random graphs across n=6 to n=10, and doing 5 trials for each n.
...
- I will be using all 15 implementations of `compute_maximal_partial_clique` in my simulation study.

- 4) What do you hope to learn with your simulation study? Alternatively, what trends/patterns/results are you hoping to see?

- I am hoping to be able to learn more holistically how each implementation performs (returns correct maximal partial clique) as n increases from 6 to 10.

- 5) How many trials do you intend to run per "level"?

- I intend to do5 trials per "level" (n).

- 6) How much time do you estimate your simulation study will need at maximum? See Note 2 below.

- I estimate that my simulation study will need at most 30 seconds * 15 implementations * 5 trials * 5 levels  = 7 1/2 minutes per trial, where 10 trials will take about at most 3.125 hour. In practice, this is unlikely to be reached as I am going through levels of n that is relatively small.
...
- The best way to make sure my code will work when running the complete simulation study is to make sure the parameters are within the range/bounds as expected, as the implementations will not work without proper input.
...
```{r}
# Use knitr's include_graphics() to display a PNG
knitr::include_graphics("HW4_check.png")
```

- As promisted from last homework, I swapped OS and fixed all bugs that existed before due to ARM incompaitbility. All output from `devtools::check()` looks clean.
...
- Implementation 2 and 7 start to struggle as n increases, while 12 does better with higher n.
...
- My answer has not changed,as I don't have any previous R projects due to limited coding courses in R, I will use the default package for the final project.
...
- Would be a good idea to add error handling into the course, especially since implementation 7 did not always work as expected (often times giving out of bound error, as it returns clique id that is higher than the max).
