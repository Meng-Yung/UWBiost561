...
cb_dim <- dim(country_both)
```

- There are `r cb_dim[1]` rows and `r cb_dim[2]` columns in country_both.
...
- Generate_random_graph() takes in three parameters, n, clique_fraction, and density_low. Then, it fills a nxn matrix with zero and ones based on density_low, symmetrizing the matrix afterwards. Then, it sets a group of nodes based on the clique_fraction slip to all be connected to each other. After that, it shuffles the nodes into a random order. Finally, it returns the shuffled network as the resulting nxn matrix.
...
- The output of `pivot_longer_heatmap()` relate to `adj_mat` as the `Value` in each row of the pivot_longer_heatmap() output is related to the values in `adj_mat` read from left to right, starting at the first row and going forward. (i.e. first 20 value in the first row of `adj_mat` is the first 20 rows of the out of `pivot_longer_heatmap()` ).
...
- This plot relates to `adj_mat` as it is plotting the 0 and 1 exactly as seen in `adj_mat`, but flipped by the X and Y axis. This is the result of having our X and Y as positive values and therefore on the plot it will plot from the bottom left towards the top right.
...
- This gave us the unshuffled matrix that was generated. The dense set of red block is the clique that was intentionally created when we were generating `adj_mat`.
...
- I would set up some / all of the following as quick unit tests for the function:
-1. Test if function is giving out the expected value for the Maximal Clique for a generated small graph.
-2. Test on graphs with no cliques larger than 2 as an edge case.
-3. Test on an empty graph.
-4. Cross compare with other/ previously established methods that is known to work. 
